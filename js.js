...
const map = {
     ...
     */
    render(snakePointsArray, foodPoint, bodysBarrier) {
		...
		// Получаем элемент ячейки с препятствием по точке barrierPoint.

		bodysBarrier.forEach((point, idx) => {
            // Получаем элемент ячейки змейки по точке point.
...
const counter = {
     ...
     */
    init() {
        this.countConteiner = document.getElementById('account');
        this.countBegin();
    },
     ...
     */
    render() {
        this.countConteiner.innerHTML = this.count;
    }
};
...
const game = {
    ...
    reset() {
		...
		this.barrier.setBarrierCoordinates(this.getRandomFreeCoordinates());
    ...
    tickHandler() {
        ...
        if (this.food.isOnPoint(this.snake.getNextStepHeadPoint())) {
            ...
            this.counter.countWork();
			//усдовие появления новых препятствий
        if(this.counter.count % 2 === 0){
    ...
    render() {
		...
		this.barrier.getBarrierCoordinates()			   );
    },
    ...
    getRandomFreeCoordinates() {
        ...
        const exclude = [this.food.getCoordinates(), this.barrier.getBarrierCoordinates(), ...this.snake.getBody()];
